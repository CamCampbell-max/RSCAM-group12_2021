...
% 
% perturb_B(16,8) = 0.05;
% perturb_B(16,16) = 0.05;
% perturb_B(16,24) = 0.05;
% 
% perturb_B(24,8) = 0.05;
% perturb_B(24,16) = 0.05;
% perturb_B(24,24) = 0.05;

%perturb_B(13,14) = +0.02;
%perturb_B(17,16) = -0.02
...
% H1
main_diag = ones([1,n]) * (1 + 8*beta);
off_diag = ones([1,n-1]) * -2*beta;
...
for day = 2:no_days
    ...
    main_diag = ones([1,n^2]) + rho_1D * alpha;
    %main_diag = ones([1,n^2]) * (1 - 4*mu - w*dt);
    
    off_diag = zeros([1,n^2 - 1]) * mu;
    far_off_diag = zeros([1,n^2 - n]) * mu;
    ...
    for k = 2:(n+1)
        for l = 2:(n+1)
            A1(k-1,l-1) =  - 1/A(k,l) * (A(k,l+1) - A(k,l-1));
            A2(k-1,l-1) =  + 1/A(k,l) * (A(k,l+1) - A(k,l-1));
            A3(k-1,l-1) =  - 1/A(k,l) * (A(k+1,l) - A(k-1,l));
            A4(k-1,l-1) =  + 1/A(k,l) * (A(k+1,l) - A(k-1,l));
        end
    end
...
%     end
    ...
    % solving b system
    % B_next = C1 \ (C2*B + alpha*(rho_vec.*A0_alt) + B_bc_adj); with bc
...
exportgraphics(hfig,'cam_Output.png','Resolution',300);
...
filename = 'Cam_method.gif'; % Specify the output file name
for idx = 1:no_days
